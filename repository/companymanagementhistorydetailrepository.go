...
func (db *CompanyManagementHistoryDetailConnection) CountCompanyManagementHistoryDetailAll(companyManagementHistoryId int) (count int64, err error) {
	res := db.connection.Debug().Table("company_management_history_details").Where("company_management_history_id = ? AND deleted_at = 0", companyManagementHistoryId).Count(&count)
