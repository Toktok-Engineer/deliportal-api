...
func (db *CompanyShareholderHistoryDetailConnection) CountCompanyShareholderHistoryDetailAll(companyShareholderHistoryId int) (count int64, err error) {
	res := db.connection.Table("company_shareholder_history_details").Where("company_shareholder_history_id = ? AND deleted_at = 0", companyShareholderHistoryId).Count(&count)
