...
func (db *EmployeeLeaveCreditConnection) CountEmployeeLeaveCreditAll(year int) (count int64, err error) {
	res := db.connection.Table("employee_leave_credits").Where("period_year = ? AND deleted_at = 0", year).Count(&count)
...
func (db *EmployeeLeaveCreditConnection) CountEmployeeLeaveCreditEmpID(year int, empId int) (count int64, err error) {
	res := db.connection.Table("employee_leave_credits").Where("employee_id = ? AND period_year = ? AND deleted_at = 0", empId, year).Count(&count)
	return count, res.Error
}
...
// 	final := "%" + strings.ToLower(search) + "%"
// 	res := db.connection.Table("employee_leave_credits").Where("(lower(period_year) LIKE ? OR lower(remark) LIKE ?) AND deleted_at = 0", final, final).Count(&count)
// 	return count, res.Error
// }
...
// 	final := "%" + strings.ToLower(search) + "%"
// 	res := db.connection.Table("employee_leave_credits").Where("lower(period_year) LIKE ?", final).Count(&count)
// 	return count, res.Error
// }
