...
func (db *CompanyGroupCompanyConnection) CountCompanyGroupCompanyAll(companyGroupID int) (count int64, err error) {
	res := db.connection.Table("company_group_companies").Where("company_group_id = ? AND deleted_at = 0", companyGroupID).Count(&count)
...
func (db *CompanyGroupCompanyConnection) FindCompanyGroupCompanys(companyGroupID int) (companyGroupCompanyOutput []model.SelectCompanyGroupCompanyParameter, err error) {
	...
	res := db.connection.Table("company_group_companies").Select("company_group_companies.id, company_group_companies.company_group_id, company_groups.company_group_name, company_group_companies.company_id, companies.company_name, company_group_companies.remark, company_group_companies.created_user_id, company_group_companies.updated_user_id, company_group_companies.deleted_user_id, company_group_companies.created_at, company_group_companies.updated_at, company_group_companies.deleted_at").Joins("left join company_groups ON company_group_companies.company_group_id = company_groups.id").Joins("left join companies ON company_group_companies.company_id = companies.id").Where("company_group_companies.company_group_id = ? AND company_group_companies.deleted_at = 0 AND companies.deleted_at = 0", companyGroupID).Order("companies.company_name").Find(&company_group_companies)
	return company_group_companies, res.Error
}
...
func (db *CompanyGroupCompanyConnection) FindCompanyGroupCompanyById(id uint) (companyGroupCompanyOutput model.SelectCompanyGroupCompanyParameter, err error) {
	...
	res := db.connection.Table("company_group_companies").Select("company_group_companies.id, company_group_companies.company_group_id, company_groups.company_group_name, company_group_companies.company_id, companies.company_name, company_group_companies.remark, company_group_companies.created_user_id, company_group_companies.updated_user_id, company_group_companies.deleted_user_id, company_group_companies.created_at, company_group_companies.updated_at, company_group_companies.deleted_at").Joins("left join company_groups ON company_group_companies.company_group_id = company_groups.id").Joins("left join companies ON company_group_companies.company_id = companies.id").Where("company_group_companies.id=? AND company_group_companies.deleted_at = 0 AND companies.deleted_at = 0", id).Take(&companyGroupCompany)
