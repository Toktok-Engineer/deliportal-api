...
func (db *CompanyAkteConnection) CountCompanyAkteAll(companyId int) (count int64, err error) {
	res := db.connection.Table("company_aktes").Where("company_id = ? AND deleted_at = 0 ", companyId).Count(&count)
