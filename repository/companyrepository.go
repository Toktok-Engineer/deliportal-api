...
func (db *CompanyConnection) UpdateCompanyApprove(company model.Company, id uint) (companyOutput model.Company, err error) {
	res := db.connection.Model(&company).Where("id=?", id).Updates(map[string]interface{}{"status": company.Status, "approved_user_id": company.ApprovedUserID, "approved_date": company.ApprovedDate, "updated_user_id": company.UpdatedUserID, "updated_at": company.UpdatedAt})
	return company, res.Error
}

func (db *CompanyConnection) UpdateCompanyDeactive(company model.Company, id uint) (companyOutput model.Company, err error) {
	res := db.connection.Model(&company).Where("id=?", id).Updates(map[string]interface{}{"status": company.Status, "deactived_user_id": company.DeactivedUserID, "deactived_date": company.DeactivedDate, "updated_user_id": company.UpdatedUserID, "updated_at": company.UpdatedAt})
	return company, res.Error
}
