...
func (db *CompanyConnection) CountCompanyAll() (count int64, err error) {
	res := db.connection.Debug().Table("companies").Where("deleted_at = 0").Count(&count)
...
func (db *CompanyConnection) CountCompanyApprove(companyID string) (count int64, err error) {
	compId := strings.Split(companyID, ",")
	res := db.connection.Debug().Table("companies").Where("companies.id NOT IN (?) AND companies.status = 2 AND deleted_at = 0", compId).Count(&count)
	return count, res.Error
}
